...
class Simple_Debug_Settings_Page{
	 ...
	 * @var string
	 */
	private $option_name = "simple-debug-settings";
	...
	private function merge_options($input){
		...
		if(count($input) > 0 && count($options) > 0){
	...
	function do_checkbox( $args ) {
		...
		$html = sprintf( '<input type="checkbox" class="checkbox" id="%5$s[%1$s][%2$s]" name="%5$s[%1$s][%2$s]" value="true"%4$s />', $args['section'], $args['id'], $value, checked( $value, 'true', false ), $this->option_name );
		$html .= sprintf( '<label for="%4$s[%1$s][%2$s]"> Enable</label><br>', $args['section'], $args['id'], $args['desc'], $this->option_name );
	 ...
	 * @return string
	 */
	private function get_section_option( $section, $option, $default = '' ) {
	 ...
	 * @param array $fields settings fields array
	 */
	public function set_sidebar( $info ) {
